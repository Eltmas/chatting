...
function sendbc(wfile) {
    if (wfile) {
        ...
        sendfile("d2bc", function () {
            ...
            send("bc", { msg: toEnglishDigits(msg), link: link });
            return;
        });
        return;
    } else {
    ...
    if (data == 1) {
        ...
        if (!$(".pallboll").find("iframe").attr("src")) {
            $(".pallboll")
                ...
                .attr(
                    "src",
                    "https://games.cdn.famobi.com/html5games/0/8-ball-billiards-classic/v250/?fg_domain=play.famobi.com&fg_aid=A-FAMOBI-COM&fg_uid=82038e98-d4e1-46dd-8de9-1460d1395eab&fg_pid=96ab9c2f-6013-4b31-96dc-ccb5c7a89329&fg_beat=976&original_ref=https%3A%2F%2Ffamobi.com%2F"
                );
        }
    }
}
...
$(function () {
    ...
    // $("#tv_box").draggable({ containment: "body", scroll: false });
});
        ...
        if (id) {
            ...
            navigator.mediaDevices.getUserMedia({ audio: true }).then(function (mediaStream) {
                ...
                mediaRecorder.onstop = function (e) {
                    ...
                    send("sendVoice", { voice: blob, id: id });
                    isrecorder = false;
                };
                isrecorder = true;
                mediaRecorder.start();
...
function newsock() {
    ...
    socket = io("", { reconnection: false, transports: trans, query: deviceget() });
    socket.on("connect", function () {
        fixSize();
        socket.emit("gh", { gh: gh });
        ...
        if (myid != null && lk != null && cpend) {
            socket.emit("rc", { token: token, n: lk });
        }
    ...
    var itv = setInterval(() => {
        if (o != null && power != null) {
            if (power.rank != o.rank || power.name != o.name || power.unick != o.unick
			|| power.alert != o.alert || power.ban != o.ban || power.bootedit != o.bootedit
			|| power.cp != o.cp || power.createroom != o.createroom || power.delbc != o.delbc
			|| power.delmsg != o.delmsg || power.delpic != o.delpic || power.edituser != o.edituser
			|| power.flter != o.flter || power.forcepm != o.forcepm || power.grupes != o.grupes
			|| power.history != o.history || power.ico != o.ico || power.kick != o.kick
			|| power.loveu != o.loveu || power.meiut != o.meiut || power.msgs != o.msgs
			|| power.mynick != o.mynick || power.owner != o.owner || power.publicmsg != o.publicmsg
			|| power.roomowner != o.roomowner || power.rooms != o.rooms || power.setpower != o.setpower
			|| power.shrt != o.shrt || power.stealth != o.stealth || power.subs != o.subs
			|| power.ulike != o.ulike || power.upgrades != o.upgrades) {
	...
	$.get("top10", function (d) {
        ...
        ltop.children().remove();
			...
			$.each(data, function (i, e) {
				if(i < 10){
			/*if(i<3){
				uh.find('.u-topic').css('text-shadow','1px 1px 1px #503930, 1px 1px 3px #604439, 1px 1px 5px #604439')
			};*/
			if(i==0){
...
function refreshonline() {
    ...
    $.get("getonline", function (d) {
        ...
        $.each(data.online, function (i, e) {
            ...
            //uh.find(".uhash").text(e.idreg.replace("#",""));
            uh.find(".u-pic").css("background-image", 'url("' + removegifpic(e.pic) + '")');
...
function fmute(e) {
    ...
    if (t.hasClass("stopmic")) {
        ...
        if (n === myid || power.createroom) {
            fm({ it: o.attr("data"), target: n, type: "hang-up" });
        }
        ...
        e.find(".evant").off().click(fmutes),
            ...
            e
                ...
                .click(function () {
                    ...
                    return getuser(myid).rep >= istalkmicro
                        ? void (
                              !ie &&
                              navigator.mediaDevices
                                  .getUserMedia({ audio: !0 })
                                  .then(function (n) {
                                      (ie = n), fq(e, !0, n), socket.emit("broadcasting", { cmd: "new", it: t }), (o.ev = !0), (o.id = myid);
                                  })
                                  .catch(function (e) {
                                      fk(e, t, myid);
                                  })
...
async function fr(e, t) {
    var o = new RTCPeerConnection({
        iceServers: [
            { urls: "turn:turn.call.yemenhost.net", username: "mobilehost", credential: "chathost" },
...
function fa(e) {
    e.candidate && fm({ type: "new-ice-candidate", it: e.currentTarget.it, target: e.currentTarget.socketId, candidate: e.candidate });
}
...
function fm(e) {
    ...
    socket.emit("broadcasting", { cmd: "send", mj: t });
}
...
function ondata(cmd, data) {
  ...
  maxrep = data;
            ...
            case "pmf":
                ...
                send("pm", { msg: "", link: data.file, id: data.id });
						};
                break;
            case "pm":
						if(myid){
            ...
            case "likemsg":
                ...
                if (islikemsg != -1) {
                    ...
                    $(".mi" + data.mi)
                        .find(".islikemsg > span")
                        .text(msglist[islikemsg].like.length);
                } else {
                    $(".mi" + data.mi)
                        .find(".islikemsg > span")
                        .text(0);
...
function login(i) {
    ...
    switch (i) {
        case 1:
            send("g", { username: $("#u1").val(), gh: gh, ss: ccode(), refr: getv("refr"), r: getv("r"), uprofile: loadprofile() });
            setv("u1", encode($("#u1").val()));
            setv("isl", "no");
            break;
        case 2:
            send("login", { username: $("#u2").val(), stealth: $("#stealth").is(":checked"), password: $("#pass1").val(), gh: gh, ss: ccode(), refr: getv("refr"), r: getv("r") });
            setv("u2", encode($("#u2").val()));
            setv("p1", encode($("#pass1").val()));
            setv("isl", "yes");
            break;
        case 3:
            send("reg", { username: $("#u3").val(), password: $("#pass2").val(), gh: gh, ss: ccode(), refr: getv("refr"), r: getv("r") });
            break;
    }
}
...
function updateu(id, uuu) {
    ...
    if (u.meiut) {
        uh.find(".muteds").toggleClass("fa-ban", true);
    } else {
        uh.find(".muteds").toggleClass("fa-ban", false);
    }
...
function sendpm(d) {
    ...
    send("pm", { msg: toEnglishDigits(m), id: d.data.uid });
}
...
function Tsend() {
    ...
    send("msg", { msg: toEnglishDigits(m), reply: isreplydone });
    norpl();
}
...
function replaymsg(data) {
    ...
    reply.html(
        '<div onclick="norpl()" class="item2 fa fa-close" style="color:#fff;position: absolute;padding: 18px;font-size: 20px !important;background:rgba(220,0,0,0.8)"></div>' +
...
function AddMsg(wid, data) {
    ...
    if (data.bid != null) {
        ...
        msg.append("<a onclick=\"send('likebc',{state:2,bid:'" + data.bid + '\'})" style="margin-top:-20px;padding:4px;margin-right:2px" class="like2 btn minix btn-danger fa fa-heart fr">&nbsp;</a>');
    }
    if (data.mi != null) {
...
function upro(id) {
    ...
    /* for (var a = ht.find(".fr.rating-box").children().removeClass().addClass("rating-star empty-star"), r = e(u.evaluation), l = 0; l <= r.start; l++)
            l == r.start ? !0 == r.h && $(a[l]).removeClass().addClass("rating-star half-star") : $(a[l]).removeClass().addClass("rating-star full-star");
		*/
    ht.find(".star").html("");
   ...
   $("#myadmin").text("أدوات إداريه");
    ...
    if (uf[(u.co || "").toLocaleLowerCase()] != null) {
        ht.find(".u-co")
            ...
            .append('<img class="fl" src="flag/' + (u.co.toLowerCase().replace("il", "ps") || "tn") + '.png">');
    } else {
        ht.find(".u-co").text("--");
    }
    var ico = getico(u);
...
function updater(r) {
    ...
    if (r.broadcast) {
        if (ht.find(".istoa").length == 0) {
            $(`<span class="istoa fa fa-microphone" style="margin: 4px 4px 0;font-size: 18px !important;color: gray;"></span>`).appendTo(ht);
        }
    } else {
...
async function fg(e) {
    try {
        ...
        await t.setRemoteDescription(o), await t.setLocalDescription(await t.createAnswer()), fm({ it: e.it, target: e.user, type: "video-answer", sdp: t.localDescription });
    } catch (e) {
        fl(e);
    }
}
